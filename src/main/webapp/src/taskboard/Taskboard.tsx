...
function Taskboard() {
  ...
  useEffect(() => {
    getAllTasks().then((response) => {
      setItemsByStatus({
        [TaskboardItemStatus.TO_DO]: response.data.filter(
          (task) => task.status == TaskboardItemStatus.TO_DO
        ),
        [TaskboardItemStatus.IN_PROGRESS]: response.data.filter(
          (task) => task.status == TaskboardItemStatus.IN_PROGRESS
        ),
        [TaskboardItemStatus.DONE]: response.data.filter(
          (task) => task.status == TaskboardItemStatus.DONE
        ),
      });
    });
  }, []);
  ...
  const handleDragEnd: DragDropContextProps['onDragEnd'] = ({
    ...
    if (destination) {
      const task =
        itemsByStatus[source.droppableId as TaskboardItemStatus][source.index];
